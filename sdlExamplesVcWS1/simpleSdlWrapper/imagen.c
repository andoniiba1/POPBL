...
void irudibatMarraztu(int id)
{
  
    if (id < irudiKop) {
        irudiaMarraztu(irudiak[id].texture, &irudiak[id].dest);
    }
}
...
void irudidenakKendu(void)
{
    int i = 0, pos = 0;

    while (irudiKop > 1) {
       // pos = irudiarenPosizioaAurkitu(0);

        //if (pos != -1) {
            SDL_DestroyTexture(irudiak[1].texture);
            for (i = 0; i < irudiKop; i++)
            {
